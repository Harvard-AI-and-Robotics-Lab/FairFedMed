...
# params for SVLoRA
ATTRIBUTE_TYPE=ethnicity  # language, race, ethnicity
...
do
  ...
  do
    for SEED in 1
    do
      DIR=output/FairLoRA_${CFG}_oct_ema/${DATASET}_${MODALITY_TYPE}_${PARTITION}_beta${BETA}_${LoRA_TYPE}_rank${LoRA_RANK}_alpha${LoRA_ALPHA}_itv4_slice${DIM_PER_3D_SLICE}/${MODEL}_${TRAINER}_${OT}_${TOP_PERCENT}_eps${EPS}_${ATTRIBUTE_TYPE}_onehotattr0.7/nctx${NCTX}_csc${CSC}_ctp${CTP}/iid_${IID}_${USERS}users_${FRAC}frac_lr${LR}_${ROUND}round_seed${SEED}_shared_half_s_${shared_half_s}
      if [ -d "$DIR" ]; then
