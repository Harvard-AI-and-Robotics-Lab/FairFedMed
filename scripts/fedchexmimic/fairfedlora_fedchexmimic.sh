...
do
  ...
  do
    for SEED in 1
    do
      DIR=output/fedchexmimic/FairLoRA_${CFG}_ema/${DATASET}_${PARTITION}_beta${BETA}_${LoRA_TYPE}_rank${LoRA_RANK}_alpha${LoRA_ALPHA}_sinit_cycle_shift/${MODEL}_${TRAINER}_${OT}_${TOP_PERCENT}_eps${EPS}_${ATTRIBUTE_TYPE}_lambda_fairness${lambda_fairness}_onehotattr0.7/nctx${NCTX}_csc${CSC}_ctp${CTP}/iid_${IID}_${USERS}users_${FRAC}frac_lr${LR}_${ROUND}round_seed${SEED}_shared_half_s_${shared_half_s}_${timestamp}
      if [ -d "$DIR" ]; then
